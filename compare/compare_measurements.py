...
def shift_peaks(df:pd.DataFrame, pre_time=100, post_time=500, time_step=1e-5):
    ...
    for i, col in enumerate(df.columns):
        # if start_index[i] <0 or end_index[i]< len(df[col]):
        #     new_data[col] =  df[col].to_list()[100: post_time+ pre_time + 1]
        # else:
        new_data[col] = df[col].to_list()[start_index[i]:end_index[i]]
    
    new_data["time"]= [i/100000 for i in range(pre_time+post_time +1)]
    for i in new_data.keys():
        new_data[i].append([0 for _ in  601 - len(new_data[i])])
        print(len(new_data[i]),i)
